...
const CyclingwithFriendContext = React.createContext()
...
export const CyclingwithFriendProvider = ({children}) => {
    const [state, dispatch] = useReducer(reducer, [])

    const addPlayer = async (room_id, id, image, name, time, distance, avgspd, callback) => {
    ...
    const editPlayer = async (room_id, id, image, name, time, distance, avgspd) => {
        await dispatch({ type: 'edit_player', payload: {room_id, id: id, image: image, name: name, time: time, distance: distance, avgspd: avgspd} })
        if (callback){
            callback()
        }
    }

    return <CyclingwithFriendContext.Provider value={{cycling: state, funct: {addPlayer, editPlayer}}}>
        {children}
    </CyclingwithFriendContext.Provider>
}

export default CyclingwithFriendContext
